...
const theme=await read('dist/news/assets/css/fmb-news-theme.css');
for(const token of ['--fmb-theme-bg:#f4efe6','--fmb-theme-bg:#0e1213','--fmb-theme-accent:#a91d31','--fmb-theme-accent:#d42a43'])must(theme.includes(token),`Editorial theme regression: missing ${token}`);
...
console.log(`FMBNews verification passed: black/ivory/red FMB NEWS. editorial home, real story-led mobile hero, text section rail, persistent five-item dock, System/Light/Dark theme runtime, ${crosswordLayout.length}-entry secure crossword, ${explainerCount} Explainer topics, and shell coverage across ${builtPageCount} pages.`);