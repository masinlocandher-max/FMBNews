...
const required = [
  ...
  'public/assets/css/fmb-news-reference-final.css','public/assets/css/fmb-news-product-identity.css','public/assets/css/fmb-news-landing-hardfix.css','public/assets/css/fmb-news-mobile-first-site.css','public/assets/css/fmb-news-mobile-personalization.css','public/assets/css/fmb-news-mobile-premium.css','public/assets/css/fmb-news-mobile-home.css','public/assets/css/fmb-news-mobile-global.css','public/assets/css/fmb-news-mobile-features.css',
  'public/assets/images/brand/fmb-bulletin-emblem.svg','public/assets/js/fmb-news-cms.js','public/assets/js/fmb-explained-library.js','public/assets/js/fmb-news-mobile-personalization.js','public/assets/js/fmb-news-mobile-premium.js','public/assets/js/fmb-news-mobile-home.js','public/assets/js/fmb-news-mobile-global.js','public/assets/js/fmb-news-weekly-horoscope.js','public/assets/js/fmb-news-weekly-crossword.js','public/assets/data/fmb-explained','content/news/articles',
  ...
  'dist/news/assets/css/fmb-news-mobile-premium.css','dist/news/assets/css/fmb-news-mobile-home.css','dist/news/assets/css/fmb-news-mobile-global.css','dist/news/assets/css/fmb-news-mobile-features.css','dist/news/assets/js/fmb-news-mobile-premium.js','dist/news/assets/js/fmb-news-mobile-home.js','dist/news/assets/js/fmb-news-mobile-global.js','dist/news/assets/js/fmb-news-weekly-horoscope.js','dist/news/assets/js/fmb-news-weekly-crossword.js','dist/news/assets/images/brand/fmb-bulletin-emblem.svg'
];
for (const rel of required) await access(resolve(rel));

try { await access(resolve('vercel.json')); throw new Error('FMBNews must not contain Vercel deployment configuration'); }
...
const mobileGlobalCss = await readFile(resolve('dist/news/assets/css/fmb-news-mobile-global.css'),'utf8');
const mobileGlobalJs = await readFile(resolve('dist/news/assets/js/fmb-news-mobile-global.js'),'utf8');
...
for (const shard of shards) {
  const entries = JSON.parse(await readFile(resolve('public/assets/data/fmb-explained',shard),'utf8'));
  if (!Array.isArray(entries)) throw new Error(`Invalid explainer shard ${shard}`);
  explainerCount += entries.length;
...
async function scanBuilt(target) {
  const info = await stat(target);
  if (info.isDirectory()) { for (const e of await readdir(target)) await scanBuilt(path.join(target,e)); return; }
  if (!/\.(?:html|css|js|mjs|json|xml|txt|svg)$/i.test(target)) return;
  const text = await readFile(target,'utf8');
  if (/(?<!\/news)\/assets\//.test(text)) throw new Error(`Unscoped root asset in ${path.relative(root,target)}`);
