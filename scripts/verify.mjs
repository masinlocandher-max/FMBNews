...
const required=[
  'site/index.html','site/fmb-brief/index.html','site/world/index.html','site/explainer/index.html','site/horoscope/index.html','site/crossword/index.html',
  'public/assets/css/fmb-news-mobile-premium.css','public/assets/css/fmb-news-mobile-home.css','public/assets/css/fmb-news-mobile-global.css','public/assets/css/fmb-news-mobile-features.css',
  'public/assets/js/fmb-news-mobile-personalization.js','public/assets/js/fmb-news-mobile-global.js','public/assets/js/fmb-news-weekly-horoscope.js','public/assets/js/fmb-news-weekly-crossword.js',
  ...
  'dist/news/index.html','dist/news/archive/index.html','dist/news/world/index.html','dist/news/explainer/index.html','dist/news/fmb-brief/index.html','dist/news/horoscope/index.html','dist/news/crossword/index.html',
  ...
  'dist/news/assets/css/fmb-news-mobile-global.css','dist/news/assets/css/fmb-news-mobile-features.css','dist/news/assets/js/fmb-news-weekly-crossword.js'
];
...
const pages={
  news:await read('dist/news/archive/index.html'),world:await read('dist/news/world/index.html'),explainer:await read('dist/news/explainer/index.html'),brief:await read('dist/news/fmb-brief/index.html'),horoscope:await read('dist/news/horoscope/index.html'),crossword:await read('dist/news/crossword/index.html')
};
for(const[name,html]of Object.entries(pages)){
  ...
  must(html.includes('fmb-news-mobile-global.css?v=20260901-global-v2'),`${name}: global mobile CSS v2 missing`);
  must(html.includes('fmb-news-mobile-global.js?v=20260901-global-v2'),`${name}: global mobile runtime v2 missing`);
}
...
async function scan(target){const info=await stat(target);if(info.isDirectory()){for(const e of await readdir(target))await scan(path.join(target,e));return}if(path.basename(target)!=='index.html')return;const text=await readFile(target,'utf8');builtPageCount++;must(text.includes('fmb-news-mobile-global.css?v=20260901-global-v2'),`Global mobile CSS not injected in ${path.relative(root,target)}`);must(text.includes('fmb-news-mobile-global.js?v=20260901-global-v2'),`Global mobile runtime not injected in ${path.relative(root,target)}`);must(!text.includes('/news/news/assets/'),`Double-scoped asset in ${path.relative(root,target)}`)}
await scan(resolve('dist/news'));
console.log(`FMBNews verification passed: localized approved hero/mug, readable unified mobile UI, 12-icon horoscope, ${entryCount}-entry current-events crossword with no active reveals, ${explainerCount} Explainer topics, and global shell across ${builtPageCount} pages.`);
