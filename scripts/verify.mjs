...
for(const[name,html]of Object.entries(pages)){
  ...
  must(html.includes('fmb-news-mobile-product-heroes.css?v=20260902-product-heroes-v3'),`${name}: strict three-product hero CSS v3 missing`);
  must(html.includes('fmb-news-mobile-menu-holder.css?v=20260902-menu-holder-v1'),`${name}: premium menu holder CSS missing`);
...
for(const token of ['One exact hero canvas','min-height:300px','padding:28px 22px 26px','.fmb-product-signal','font-size:46px','Same bottom capsule','fmb-mobile-route-world .world-hero','fmb-mobile-route-explainer .explainer-hero','fmb-mobile-route-brief .brief-archive-hero'])must(heroCss.includes(token),`Strict three-product hero regression: missing ${token}`);
