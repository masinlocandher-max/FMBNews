...
const required=[
  ...
  'public/assets/css/fmb-news-mobile-premium.css','public/assets/css/fmb-news-mobile-home.css','public/assets/css/fmb-news-mobile-global.css','public/assets/css/fmb-news-mobile-products.css','public/assets/css/fmb-news-mobile-app-polish.css','public/assets/css/fmb-news-mobile-home-live-hero.css','public/assets/css/fmb-news-mobile-contrast-lock.css','public/assets/css/fmb-news-mobile-features.css','public/assets/css/fmb-news-mobile-personalization.css',
  ...
  'dist/news/assets/css/fmb-news-mobile-global.css','dist/news/assets/css/fmb-news-mobile-products.css','dist/news/assets/css/fmb-news-mobile-app-polish.css','dist/news/assets/css/fmb-news-mobile-home-live-hero.css','dist/news/assets/css/fmb-news-mobile-contrast-lock.css','dist/news/assets/css/fmb-news-mobile-features.css','dist/news/assets/css/fmb-news-mobile-personalization.css',
...
for(const[name,html]of Object.entries(pages)){
  ...
  must(html.includes('fmb-news-mobile-home-live-hero.css?v=20260902-live-hero-v1'),`${name}: full-bleed Home hero stylesheet missing`);
...
async function scan(target){const info=await stat(target);if(info.isDirectory()){for(const e of await readdir(target))await scan(path.join(target,e));return}if(path.basename(target)!=='index.html')return;const text=await readFile(target,'utf8');builtPageCount++;must(text.includes('fmb-news-mobile-personalization.css?v=20260901-personal-v3'),`Personalization CSS v3 not injected in ${path.relative(root,target)}`);must(text.includes('fmb-news-mobile-global.css?v=20260901-global-v3'),`Global mobile CSS not injected in ${path.relative(root,target)}`);must(text.includes('fmb-news-mobile-global.js?v=20260901-global-v3'),`Global mobile runtime not injected in ${path.relative(root,target)}`);must(text.includes('fmb-news-mobile-products.css?v=20260901-products-v1'),`Dedicated product CSS not injected in ${path.relative(root,target)}`);must(text.includes('fmb-news-mobile-products.js?v=20260901-products-v2'),`Dedicated product runtime v2 not injected in ${path.relative(root,target)}`);must(text.includes('fmb-news-mobile-app-polish.css?v=20260902-polish-v2'),`Final app polish CSS v2 not injected in ${path.relative(root,target)}`);must(text.includes('fmb-news-mobile-app-polish.js?v=20260902-polish-v2'),`Final app polish runtime v2 not injected in ${path.relative(root,target)}`);must(text.includes('fmb-news-mobile-home-live-hero.css?v=20260902-live-hero-v1'),`Live Home hero CSS not injected in ${path.relative(root,target)}`);must(text.includes('fmb-news-mobile-contrast-lock.css?v=20260902-contrast-v1'),`Contrast lock not injected in ${path.relative(root,target)}`);must(!text.includes('/news/news/assets/'),`Double-scoped asset in ${path.relative(root,target)}`)}
await scan(resolve('dist/news'));
console.log(`FMBNews verification passed: localized approved hero/mug, one premium mobile masthead and product rail, full-bleed live Home hero with dynamic greeting and large date/weather, final contrast lock, immediate internal product content, eight dedicated route designs, 12-icon horoscope, ${entryCount}-entry current-events crossword with no active reveals, ${explainerCount} Explainer topics, and full FMB shell coverage across ${builtPageCount} pages.`);
