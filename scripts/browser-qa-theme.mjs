...
const mobileShell=mobilePage.locator('.fmb-mobile-app-shell');
await mobileShell.first().waitFor({state:'attached'});
assert.equal(await mobileShell.count(),1,'Mobile Home must expose exactly one editorial shell.');
assert.equal(await mobilePage.evaluate(()=>matchMedia('(max-width:699px)').matches),true,'iPhone QA context must match the mobile stylesheet breakpoint.');
assert.equal(await mobileShell.evaluate(el=>getComputedStyle(el).backgroundColor),'rgb(11, 15, 17)','Mobile editorial masthead must remain newsroom black.');
