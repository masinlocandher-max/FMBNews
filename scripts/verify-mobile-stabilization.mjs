...
// The dock's own structure, asserted directly.
...
// The section rail is part of the navigation again: the publication's desks,
// named, under the wordmark, including Fact Check which the dock has no room
// for. The hamburger is not, and that distinction is the whole point -- it
// opened the same panel as the dock's Menu, the same element with the same
// links, so a reader got the identical forty destinations from two controls on
// one screen. Matched against generated markup, not file text, so the comments
// explaining this do not themselves trip the guards.
must(globalJs.includes('class="fmb-mobile-product-rail"'),'The mobile section rail must be built by the shared shell runtime');
must(globalJs.includes("['fact','/news/fact-check/','Fact Check']"),'The section rail must carry Fact Check, which the five-item dock has no room for');
...
// The rail's rendered geometry is asserted in the browser suite, where it can
// be measured, rather than by matching a CSS rule here.
...
console.log('Mobile stabilization contracts passed: one PHT runtime, FMB NEWS. masthead over a wordmark-and-search app bar, story-led Home, the named section rail under the wordmark, the five-item editorial dock, and no hamburger duplicating the dock Menu, accessible menu sheets, reduced motion, and route-wide readability.');