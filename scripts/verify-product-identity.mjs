...
function expected(rel){
  ...
  if(p.startsWith('explainer/'))return{title:'FMB Explainer',cls:'fmb-explainer-route',kind:'explainer'};
...
for(const file of pages){
  ...
  if(exp.kind==='landing'){
    ...
    for(const signal of ['Trusted News.','Meaningful Perspectives.','<h2>FMB News</h2>','<h2>FMB Worldwide</h2>','<h2>FMB Explainer</h2>','<h2>FMB Daily Brief</h2>','Explore FMB News','Explore Worldwide','Open Explainer','Continue with Email']){
      if(!html.includes(signal))throw new Error(`${rel}: approved landing content missing ${signal}`);
    }
    for(const retired of ['One publication. Three focused products.','Clear reporting without the noise.','Verified first','Context included','Useful by design','Philippines · Explainers · Overviews','World · Explainers · Overviews']){
...
console.log(`Product identity verification passed across ${checked} pages: Filipino Media Bulletin with FMB News, FMB Worldwide, FMB Explainer, FMB Daily Brief, and non-redundant footer.`);
