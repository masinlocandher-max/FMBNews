...
for(const rel of [
  ...
  'dist/news/assets/css/fmb-news-editorial-reference-v2.css',
...
for(const signal of ['--landing-paper:#f4efe6','--landing-ink:#111417','--landing-red:#a91d31','.network-hero-art{display:none!important}','.network-products','.daily-brief-signup','.publication-footer'])if(!landingCss.includes(signal))throw new Error(`Landing visual-system regression: missing ${signal}`);
...
for(const signal of ['--fmb-red:#a61f32','.fmb-editorial-wordmark','.editorial-status-strip','.editorial-top-grid','.editorial-side-rail','.editorial-bottom-grid','.founder-card'])if(!referenceCss.includes(signal))throw new Error(`Editorial reference regression: missing ${signal}`);
...
for(const file of pages){
  ...
  if(exp.kind==='landing'){
    for(const signal of [
      ...
      '/news/assets/css/fmb-news-editorial-reference-v2.css',
      ...
      'fmb-editorial-wordmark',
      'FMB NEWS<span class="dot">.</span>',
      'Filipino Media Bulletin</span>',
      'News. Worldwide. Sports.',
      'class="editorial-lead-story"',
      'class="editorial-side-rail"',
    ...
    if((html.match(/class="network-product /g)||[]).length!==5)throw new Error(`${rel}: landing must expose exactly five editorial product modules`);
    if((html.match(/class="editorial-desk /g)||[]).length!==3)throw new Error(`${rel}: landing must expose exactly three direct editorial desks`);
    ...
    if(!html.includes('<div class="footer-publication-title">FMB NEWS.</div>'))throw new Error(`${rel}: editorial landing footer must use FMB NEWS.`);
  }
  if(exp.kind==='sports'){
  ...
  if(exp.kind!=='landing'&&!html.includes('<div class="footer-publication-title">Filipino Media Bulletin</div>'))throw new Error(`${rel}: internal footer publication is not Filipino Media Bulletin`);
...
console.log(`Product identity verification passed across ${checked} pages: FMB NEWS. editorial reference, News/Worldwide/Sports desk IA, five publication products, Entertainment grouping, founder identity, inventory-aware Sports, and non-redundant footer.`);