...
for(const signal of ['--fmb-red:#a61f32','.fmb-editorial-wordmark','.editorial-status-strip','.editorial-top-grid','.editorial-side-rail','.network-products','.editorial-desks','.editorial-bottom-grid','.founder-card','.daily-brief-signup'])if(!referenceCss.includes(signal))throw new Error(`Approved editorial reference regression: missing ${signal}`);
...
for(const file of pages){
  ...
  if(exp.kind==='landing'){
    for(const signal of [
      ...
      'fmb-editorial-subtitle">Filipino Media Bulletin',
