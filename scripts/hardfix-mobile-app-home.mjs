...
function storyRow(s){return `<a class="fmb-app-story-row" href="/news/${esc(s.slug)}/"><img src="${esc(imageFor(s))}" alt="${esc(s.image?.alt||s.headline)}" loading="lazy"><div class="fmb-app-story-copy">${meta(s)}<h3>${esc(s.headline)}</h3></div></a>`}
...
const latest=ss.slice(0,5),breaking=breakingStory(ss),tickerStories=ss.slice(0,4);
...
console.log(`Injected the approved metallic violet FMB mobile home with one ${tickerLabel.toLowerCase()} rail, the current cinematic hero, live date/time/weather, no duplicate lead hero, feature cards, and ${latest.length} latest stories.`);