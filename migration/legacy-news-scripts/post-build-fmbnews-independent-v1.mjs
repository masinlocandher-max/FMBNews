...
const cssSource = path.join(root, 'apps', 'withlovefmb', 'assets', 'css', 'fmbnews-independent-v1.css');
const cssDestination = path.join(dist, 'assets', 'css', 'fmbnews-independent-v1.css');
const sitemapPath = path.join(dist, 'sitemap.xml');
const version = '20260803-independent-v1';
const categories = [
  ['money', 'Money'],
  ['tech', 'Tech'],
  ['lifestyle', 'Lifestyle'],
...
function publicationHeader(active = '') {
  return `<header class="nc-site-header fn-site-header" id="top">
    <div class="fn-header-inner">
      <a class="fn-brand" href="/fmbnews/" aria-label="FMB News home">
        <span class="fn-brand-name">FMB NEWS</span>
        <span class="fn-brand-line">The news, made clear.</span>
      </a>
      <nav class="nc-site-links fn-header-links" id="newsNav" aria-label="FMB News navigation">
        <a href="/fmbnews/"${active === 'news' ? ' aria-current="page"' : ''}>Latest</a>
        <a href="/news/about/"${active === 'about' ? ' aria-current="page"' : ''}>About</a>
        <a class="fn-home-button" href="/fmbandco/">FMB&amp;CO. Home</a>
      </nav>
      <button class="nc-menu-toggle fn-menu-toggle" type="button" data-news-menu aria-label="Open news menu" aria-expanded="false" aria-controls="newsNav"><span></span><span></span></button>
    </div>
  </header>`;
}
...
for (const file of files) {
  ...
  html = addBodyClass(html, 'fn-news-independent')
    .replace(/<link\b[^>]*data-fmb-news-independent[^>]*>\s*/gi, '')
    .replace(/<header\b[^>]*class=(['"])[^'"]*\bnc-site-header\b[^'"]*\1[^>]*>[\s\S]*?<\/header>/i, publicationHeader(landingFiles.has(file) ? 'news' : ''))
    .replace(/<nav\b[^>]*class=(['"])[^'"]*\bnc-topic-rail\b[^'"]*\1[^>]*>[\s\S]*?<\/nav>/i, categoryRail())
    .replace(/<footer\b[^>]*class=(['"])[^'"]*\bnc-footer\b[^'"]*\1[^>]*>[\s\S]*?<\/footer>/i, publicationFooter())
    .replace(/<\/head>/i, `<link rel="stylesheet" href="/assets/css/fmbnews-independent-v1.css?v=${version}" data-fmb-news-independent></head>`);

  if (landingFiles.has(file) && !html.includes('class="fn-intro"')) {
  ...
  if (!html.includes('class="fn-site-header"')) throw new Error(`Independent News header missing: ${file}`);
  if (!html.includes('class="fn-site-footer"')) throw new Error(`Independent News footer missing: ${file}`);
  if (!html.includes('FMB&amp;CO. Home')) throw new Error(`FMB&CO. Home button missing: ${file}`);
  if (/(?:fmb-news-official-transparent\.webp|fmb-news-official\.svg)/i.test(html.replace(/<meta\b[^>]*>/gi, ''))) {
    throw new Error(`Visible retired News logo remains: ${file}`);
  }
...
console.log(`Applied the independent text-only FMB News publication to ${updated} generated route(s) and created the short About page.`);
