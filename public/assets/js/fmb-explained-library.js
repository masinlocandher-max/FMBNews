...
function render(items){
  ...
  list.innerHTML=items.map(item=>{const pub=published.get(Number(item.id));const title=pub?.title||cleanTitle(item.title);const date=pub?`<div class="explained-source-note">FMB Explainer Archive · ${escapeHtml(archiveLabel(pub.archiveDate))}</div>`:'';const read=pub?`<a class="explained-read" href="/news/explainer/${escapeHtml(pub.articleSlug)}/">Read the full article →</a>`:'';return `<details class="explained-item" id="topic-${item.id}"><summary><span class="explained-number">${String(item.id).padStart(3,'0')}</span><span class="explained-title">${escapeHtml(title)}</span><span class="explained-plus" aria-hidden="true">+</span></summary><div class="explained-body"><section><div class="explained-label">Overview</div><p>${escapeHtml(item.explanation)}</p></section><section class="explained-why"><div class="explained-label">Why it matters</div><p>${escapeHtml(item.why)}</p></section>${date}${read}</div></details>`}).join('')
}
...
async function loadLibrary(){
  try{
    ...
    library=payloads.slice(0,-1).flat().sort((a,b)=>a.id-b.id);const index=payloads.at(-1);
    if(library.length!==206)throw new Error(`Expected 206 topics, received ${library.length}`);if(index.length!==206)throw new Error(`Expected 206 published articles, received ${index.length}`);
    published=new Map(index.map(item=>[Number(item.id),item]));if(status)status.textContent='206 full articles · FMB Explainer';render(library);
