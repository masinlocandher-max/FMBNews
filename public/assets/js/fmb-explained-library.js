...
function render(items){if(!list)return;if(count)count.textContent=`${items.length} of ${library.length} explainers`;if(!items.length){list.innerHTML='<div class="explained-no-results"><strong>No matching explainer.</strong><span>Try another word, place, policy, person, or issue.</span></div>';return}list.innerHTML=items.map(item=>`<details class="explained-item" id="topic-${item.id}"><summary><span class="explained-number">${String(item.id).padStart(3,'0')}</span><span class="explained-title">${escapeHtml(cleanTitle(item.title))}</span><span class="explained-plus" aria-hidden="true">+</span></summary><div class="explained-body"><section><div class="explained-label">Let me explain</div><p>${escapeHtml(item.explanation)}</p></section><section class="explained-why"><div class="explained-label">Why this matters to us as Filipinos</div><p>${escapeHtml(item.why)}</p></section><div class="explained-source-note">Verified source lists are attached when each explainer is finalized for publication.</div></div></details>`).join('')}
function filterLibrary(){const q=(search?.value||'').trim().toLocaleLowerCase('en-PH');if(!q)return render(library);render(library.filter(item=>`${item.title} ${item.explanation} ${item.why}`.toLocaleLowerCase('en-PH').includes(q)))}
async function loadLibrary(){try{if(status)status.textContent='Loading 206 explainers…';const responses=await Promise.all(FMB_EXPLAINED_SHARDS.map(url=>fetch(url,{cache:'no-store'})));const failed=responses.find(response=>!response.ok);if(failed)throw new Error(`Library request failed: ${failed.status}`);const chunks=await Promise.all(responses.map(response=>response.json()));library=chunks.flat().sort((a,b)=>a.id-b.id);if(library.length!==206)throw new Error(`Expected 206 entries, received ${library.length}`);if(status)status.textContent='206 explainers · FMB Explainer';render(library);const hash=location.hash.match(/^#topic-(\d+)$/);if(hash){const target=document.querySelector(location.hash);if(target){target.open=true;setTimeout(()=>target.scrollIntoView({behavior:'smooth',block:'start'}),50)}}}catch(error){console.error(error);if(status)status.textContent='FMB Explainer library unavailable';if(list)list.innerHTML='<div class="explained-no-results"><strong>The explainer library could not load.</strong><span>Please refresh the page.</span></div>'}}
